...
import pandas as pd
from firewall.firewall_interface import FirewallCollector, FirewallInterface
...
class MF2Collector(FirewallCollector, FirewallInterface):
    ...
    def export_network_objects(self) -> pd.DataFrame:
        ...
        files = download_object_files(self.device_ip, 22, self.username, self.password, '/secui/etc/', self.temp_dir)
        if len(files) < 4:
