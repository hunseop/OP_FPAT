...
class PaloAltoAPI:
    ...
    def save_dfs_to_excel(self, dfs, sheet_names, file_name):
        """
        여러 DataFrame을 엑셀 파일로 저장하고 스타일을 적용하는 함수
        """
        try:
            ...
            logging.error(f"An error occurred while saving DataFrames to Excel: {e}")
            return False
    
    def save_df_to_excel(self, df, df_name):
        """
        단일 DataFrame을 엑셀 파일로 저장하는 함수
        """
    ...
    def get_member(self, entry):
        """
        XML entry에서 member 요소를 추출하는 함수

        :param entry: XML entry
        :return: member 요소의 텍스트 리스트
        """
        try:
            return [member.text for member in entry]
        except Exception as e:
            logging.error(f"An error occurred while getting members: {e}")
            return []

    def list_to_string(self, list_data):
        """
        리스트를 콤마로 구분된 문자열로 변환하는 함수
    ...
    def get_vsys_list(self):
        ...
        return [vsys.attrib.get('name') for vsys in root]
    ...
    def export_security_rules(self, config_type: str = 'running'):
        ...
        for vsys in vsys_list:
            ...
            for idx, rule in enumerate(rulebase):
                ...
                description = self.list_to_string([i.replace('\n', ' ') if isinstance(i, str) else i for i in self.get_member(rule.findall('./description'))])
    ...
    def export_network_objects(self, config_type: str = 'running'):
        ...
        for address in address_list:
            ip_name = address.attrib.get('name')
            address_type = address.find('*').tag
            ip_member = [j.text for j in address.findall(f'./{address_type}')]
    ...
    def export_network_group_objects(self, config_type: str = 'running'):
        ...
        for address_group in address_group_list:
            group_name = address_group.attrib.get('name')
            group_member = [j.text for j in address_group.findall('./static/member')]
    ...
    def export_service_group_objects(self, config_type: str = 'running'):
        ...
        for service_group in service_groups:
            ...
            group_member = [j.text for j in service_group.findall('./members/member')]

            service_info = {
                "Group Name": group_name,
                "Entry": self.list_to_string(group_member),
            }
            service_group_objects.append(service_info)
